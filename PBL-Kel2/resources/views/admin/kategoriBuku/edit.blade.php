...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
    <div class="p-6">
        <h1 class="text-2xl font-bold mb-6">Edit Kategori Buku</h1>
        ...
        <form action="{{ route('kategori-buku.update', $kategoriBuku->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $kategoriBuku->nama) }}" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                @error('nama')
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                    <option value="1" {{ old('status', $kategoriBuku->status) == 1 ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('status', $kategoriBuku->status) == 0 ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
