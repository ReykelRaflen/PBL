    ...
    <div class="ml-64 w-full">
        ...
        <div class="flex justify-between items-center">
            <div class="text-sm text-gray-700">
                Showing 1 to 25 of {{ $books->total() }} entries
            </div>
            <div class="flex space-x-1">
                <a href="{{ $books->previousPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 {{ $books->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    Previous
                </a>
                @for ($i = 1; $i <= min($books->lastPage(), 3); $i++)
                    <a href="{{ $books->url($i) }}" class="px-4 py-2 text-sm font-medium {{ $books->currentPage() == $i ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50' }} border border-gray-300 rounded-md">
                        {{ $i }}
                    </a>
                @endfor
                <a href="{{ $books->nextPageUrl() }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 {{ !$books->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    Next
                </a>
            </div>
        </div>
