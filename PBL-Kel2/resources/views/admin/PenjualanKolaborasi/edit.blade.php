...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
    <div class="p-6">
        ...
        <form action="{{ route('penjualanKolaborasi.update', $laporan->id) }}" 
              method="POST" 
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="status_pembayaran" id="status_pembayaran"
                            ...
                            <option value="tidak_sesuai" {{ old('status_pembayaran', $laporan->status_pembayaran) == 'tidak_sesuai' || old('status_pembayaran', $laporan->status_pembayaran) == 'tidak_sesuai' ? 'selected' : '' }}>
                                Tidak Sesuai
                            </option>
                        </select>
                        ...
                        <select name="pesanan_kolaborasi_id" id="pesanan_kolaborasi_id"
                            ...
                            @foreach($pesananBuku as $pesanan)
                                <option value="{{ $pesanan->id }}"
                                    ...
                                    {{ $pesanan->nomor_pesanan ?? 'N/A' }} - {{ $pesanan->bukuKolaboratif->judul ?? 'Judul tidak ditemukan' }}
...
<script>
    ...
    document.getElementById('pesanan_kolaborasi_id').addEventListener('change', function() {
        ...
        if (pesananId) {
            console.log('Pesanan selected:', pesananId);
            // Implementasi AJAX jika diperlukan
        }
    });
