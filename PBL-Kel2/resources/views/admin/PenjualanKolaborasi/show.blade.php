...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg max-w-2xl mx-auto">
    <div class="p-6">
        <h1 class="text-2xl font-bold mb-6">Detail Laporan Penjualan Kolaborasi</h1>
        ...
        <div class="space-y-4 text-sm text-gray-800 dark:text-gray-200">
            ...
            <div>
                ...
                @if($laporan->bukti_pembayaran)
                    <img src="{{ asset('storage/bukti/' . $laporan->bukti_pembayaran) }}" class="rounded border w-64 mt-2">
                @else
                    <p class="italic text-gray-400">Tidak ada gambar bukti pembayaran.</p>
                @endif
            </div>
        </div>

        <div class="mt-6 text-right">
            <a href="{{ route('penjualanKolaborasi.index') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">
                Kembali
            </a>
        </div>
    </div>
