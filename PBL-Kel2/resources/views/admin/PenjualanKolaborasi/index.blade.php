    ...
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <div class="bg-purple-500 text-white p-4 rounded-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-bold">Rp {{ number_format($statistik['total_nilai'], 0, ',', '.') }}</h3>
                ...
                <tbody>
                    @forelse ($laporan as $item)
                        <tr class="border-t dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="p-3">
                                <div>
                                    <strong class="text-blue-600">{{ $item->nomor_invoice }}</strong>
                                    @if($item->pesananKolaborasi)
                                        <br><small class="text-gray-500">ID: {{ $item->pesananKolaborasi->nomor_pesanan }}</small>
                                    @endif
                                </div>
                            </td>
                            <td class="p-3">
                                <div>
                                    ...
                                    @if($item->pesananKolaborasi && $item->pesananKolaborasi->babBuku)
                                        <br><span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ ucfirst($item->pesananKolaborasi->babBuku->tingkat_kesulitan ?? 'Normal') }}
                                        </span>
                                    @endif
                                </div>
                            </td>
                            <td class="p-3">
                                <div>
                                    {{ $item->penulis }}
                                    @if($item->pesananKolaborasi && $item->pesananKolaborasi->user)
                                        <br><small class="text-gray-500">{{ $item->pesananKolaborasi->user->email }}</small>
                                    @endif
                                </div>
                            </td>
                            <td class="p-3">
                                ...
                                <div>
                                    ...
                                    @if($item->tanggal_verifikasi)
                                        <br><small class="text-gray-500">Verifikasi:
                                            {{ \Carbon\Carbon::parse($item->tanggal_verifikasi)->format('d/m/Y H:i') }}</small>
                                    @endif
                                </div>
                            </td>
                            <td class="p-3">
                                @php
                                    ...
                                    switch($item->status_pembayaran) {
                                        ...
                                        default:
                                            ...
                                            $statusText = ucfirst(str_replace('_', ' ', $item->status_pembayaran));
                                    }
                                @endphp
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($item->status_pembayaran === 'menunggu_verifikasi')
                                        <div class="flex gap-1">
                                            <!-- ACCEPT BUTTON -->
                                            <form action="{{ route('penjualanKolaborasi.accept', $item->id) }}" method="POST"
...
<div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
        <div class="mt-3">
            ...
            <!-- FORM REJECT - MENGGUNAKAN ROUTE REJECT -->
            <form id="rejectForm" method="POST" action="">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="catatan_admin" id="catatan_admin" rows="4"
                        ...
                        placeholder="Jelaskan alasan penolakan pembayaran secara detail..." required></textarea>
                    <small class="text-gray-500">
                        <i class="fas fa-info-circle mr-1"></i>
                        Minimal 10 karakter. Alasan ini akan dikirim ke penulis.
                    </small>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700 dark:text-yellow-300">
                                <strong>Perhatian:</strong> Menolak pembayaran akan mengembalikan bab ke status tersedia
                                dan penulis perlu melakukan pembayaran ulang.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-ban mr-2"></i>Tolak Pembayaran
                    </button>
                </div>
            </form>
        </div>
...
<script>
    function openRejectModal(id) {
        ...
        // Set form action menggunakan route reject yang benar
        const form = document.getElementById('rejectForm');
        form.action = `{{ route('penjualanKolaborasi.index') }}/${id}/reject`;
    ...
    document.getElementById('rejectForm').addEventListener('submit', function (e) {
        ...
        if (!confirm('Yakin ingin menolak pembayaran ini?\n\nBab akan dikembalikan ke status tersedia dan penulis perlu melakukan pembayaran ulang.')) {
            e.preventDefault();
            return false;
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        setTimeout(function () {
            const successAlerts = document.querySelectorAll('.bg-green-100, [class*="bg-green"]');
            successAlerts.forEach(function (alert) {
                if (alert.textContent.includes('berhasil')) {
