                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" 
                                       id="harga_ebook"
                                       name="harga_ebook" 
                                       value="{{ old('harga_ebook', $book->harga_ebook) }}"
                                       min="0"
                                       step="1000"
                        ...
                        <div>
                            ...
                            <div class="text-center">
                                <img src="{{ asset('storage/' . $book->cover) }}" alt="Current Cover" 
                                     class="w-32 h-44 object-cover rounded shadow mx-auto">
                            </div>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg hover:border-gray-400 dark:hover:border-gray-500 transition-colors">
                                <div class="space-y-1 text-center">
                                    ...
                                    @if($book->cover)
                                        <p class="text-xs text-blue-600 dark:text-blue-400">Kosongkan jika tidak ingin mengubah</p>
                                    @endif
                                </div>
                            </div>
                            @error('cover')
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg hover:border-gray-400 dark:hover:border-gray-500 transition-colors">
                                <div class="space-y-1 text-center">
                                    ...
                                    @if($book->file_buku)
                                        <p class="text-xs text-blue-600 dark:text-blue-400 mt-1">File saat ini: {{ basename($book->file_buku) }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Kosongkan jika tidak ingin mengubah</p>
                                    @endif
                                </div>
                            </div>
                            @error('file_buku')
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600 rounded-b-lg">
                <div class="flex items-center justify-between">
                    <div class="flex space-x-3">
                        ...
                        <a href="{{ route('admin.books.show', $book->id) }}" class="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-lg shadow font-medium transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline mr-2" viewBox="0 0 20 20" fill="currentColor">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Check if e-book price is set but no PDF and no existing PDF
        if (hargaEbookInput.value && pdfInput.files.length === 0 && !'{{ $book->file_buku }}') {
    ...
    // Check if existing e-book file and set required status
    const hasExistingEbook = '{{ $book->file_buku }}';
    if (hasExistingEbook && hargaEbookInput.value) {
        hargaEbookInput.required = true;
    }
});
</script>
...
.edit-mode .file-upload-zone {
    border-style: solid;
    border-width: 1px;
    border-color: #d1d5db;
}

.edit-mode .file-upload-zone:hover {
    border-color: #3b82f6;
    border-style: dashed;
    border-width: 2px;
}

/* Comparison view for current vs new files */
.file-comparison {
    display: grid;
...
.dark .file-comparison .current-file {
    background-color: #374151;
    border-color: #4b5563;
}

.dark .file-comparison .new-file {
